...
| **Keyboard RGB control** | Capability detection, field-order readback, the full UI, argument validation (10 tests including injection attempts), correct exit codes, refusal when unprivileged. This machine **does** have RGB: `kbd_rgb_mode` is present, write-only, fields `cmd mode red green blue speed`. | **No colour has been written to hardware.** `kbd_rgb_mode` is `--w------- root`, and the helper is only installed by the `.deb` or `scripts/install-privileged.sh`, both of which need root once. Until then the controls are correctly greyed out. |
| **Battery charge limit** | Support detected and the current value read without privilege; the write path is validated end to end short of the privileged write itself. | The privileged write has **not** been executed on this machine: `jamsys-power` needs installing with root once. Whether this firmware honours values other than 60/80/100 is unverified. |
